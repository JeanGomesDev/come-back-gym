...
export default function Navigation() {
  ...
  const NAV_ITEMS = [
    { href: '/', label: t.nav.hoje, icon: '🏋️' },
    { href: '/plano', label: t.nav.plano, icon: '📅' },
    { href: '/dashboard', label: t.nav.stats, icon: '📊' },
    { href: '/medidas', label: t.nav.medidas, icon: '📏' },
    { href: '/bioimpedancia', label: t.nav.bio, icon: '⚖️' },
  ];
        ...
        <div className="max-w-2xl mx-auto flex items-stretch h-16">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={`flex-1 flex flex-col items-center justify-center gap-1 transition-colors ${active ? 'text-emerald-400' : 'text-zinc-500 hover:text-zinc-300'}`}>
          ...
          <button onClick={() => setMenuOpen(true)}
            ...
            {user?.photoURL ? (
              ...
              <img src={user.photoURL} alt="" className="w-[22px] h-[22px] rounded-full ring-1 ring-zinc-600" />
            ) : (
            ...
            <div className="max-w-2xl mx-auto bg-zinc-900 border-t border-zinc-700 rounded-t-3xl overflow-hidden shadow-2xl">
              ...
              <div className="flex items-center gap-3 px-5 py-4 border-b border-zinc-800">
                {user?.photoURL ? (
                  ...
                  <div className="w-11 h-11 rounded-full bg-zinc-700 flex items-center justify-center text-xl">👤</div>
                )}
              ...
              <div className="p-3 space-y-1">
                <Link href="/configuracoes" onClick={closeMenu}
